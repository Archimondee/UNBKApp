...
export default class Register extends Component {
  ...
  submitRegister () {
    const {nama, kelas, username, password, confirm, sekolah} = this.state;
    jumlah = 0;
    nama != '' ? (jumlah += 1) : null;
    kelas != '' ? (jumlah += 1) : null;
    ...
    if (jumlah != 5) {
      ...
      if (password != confirm) {
        ...
        fetch (
          'https://kumpulan-soal-smp.000webhostapp.com/api_soal/Register.php',
          {
            method: 'POST',
            headers: {
        ...
        <View style={{flex: 1.5}}>
          <ScrollView style={{flex: 1.5, paddingBottom: 15}}>
            <Form style={{}}>
              ...
              <Item floatingLabel style={{borderBottomWidth: 0}}>
                <Label style={{color: 'black'}}>Sekolah</Label>
                <Input
                  onChangeText={sekolah => this.setState ({sekolah})}
                  style={{borderBottomColor: 'black', borderBottomWidth: 2}}
                />
              </Item>
            </Form>
